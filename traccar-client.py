...
config_candidates=[os.getenv('traccar_client_config','traccar-client.ini'),'traccar-client.ini', '/etc/traccar-client.ini']
...
def main(args):
    ...
    print("checking if Traccar sever details are configured via environment variable : 'traccar_server'")
    traccar_server = os.getenv('traccar_server')
    if(traccar_server==None):
        traccar_server= traccar_cfg.get('server','http://localhost:8080')
    else:
        print("client found the server location from environment variable as : " +traccar_server)
    ...
    while RUNNING==True:
        ...
        print("Current pos at line "+str(pos.line))
        if pos.valid:
        ...
        print("processed line ", str(pos.line))
